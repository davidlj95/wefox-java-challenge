...
8.- Create the implementation of a new Pokemons REST API to retrieve pokemons that name starts with concrete string. The business logic (retrieve the pokemons) must be executed in a Consumer of Rabbitmq queue (free name). The name of the endpoint must be '/pokemon/queue'.
it means that the logic part should be executed in a consumer of queue. The queue must be created dynamically (without human intervention).
